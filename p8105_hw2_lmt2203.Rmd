...
The "pols_month" dataset contains **`r count(pols_df)`** observations and contains information on the number of politicians who are democratic or republican at any given time. The year range from `r range(pols_df$year)`. The "snp" dataset contains **`r count(snp_df)`** observations and contains the closing values of the S&P stock index on a given date. The year range from `r range(snp_df$year)`. The "unemployment" dataset contains **`r count(unemployment_df)`** observations and has percentage of unemployment of the associated time. The year range from `r range(unemployment_df$year)`. 
The combined dataset has a total of **`r count(pols_snp_unemployment)`** observations of  **`r ncol(pols_snp_unemployment)`** variables.  
